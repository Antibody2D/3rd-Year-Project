...
void Ship::handleEvent(SDL_Event& e) {
	...
	if (e.type == SDL_KEYDOWN && e.key.repeat == 0) {
		...
		// FIRE WEAPON
		case SDLK_SPACE:
			weapon.spawnLaser();		//Added sound effect
			break; //Press space bar to spawn a new laser
		case SDLK_n:
			weapon.spawnNinjaStar();	//Added sound effect
			break; //Press 'n' to spawn a new Ninja Star
		}
	}
