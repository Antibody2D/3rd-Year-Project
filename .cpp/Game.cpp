...
bool loadMedia() {
	...
	// Load Ship texture
	if (!gShipTexture.loadFromFile("PlayerShip.png")) {
	...
	if (!gBloodCellTexture.loadFromFile("BloodCell.png")) {			// 10/01 Added Blood Cell
		printf("Failed to load Blood Cell texture!\n");
		success = false;
	}
	// Load Blood Cell texture
	if (!gBloodCellSmallTexture.loadFromFile("BloodCellSmall.png")) {			// 10/01 Added Blood Cell
		printf("Failed to load Small Blood Cell texture!\n");
		success = false;
	}
	// Load background texture
	if (!gBGTexture.loadFromFile("Background800.png")) {				// 09/01 Edited background to be 800 x 600 instead of 600 * 480
		printf("Failed to load background texture!\n");
		success = false;
	}

	// Load Green Laser texture
	if (!gLaserGreenTexture.loadFromFile("LaserGreen.png")) {
		printf("Failed to load Green Laser texture!\n");
		success = false;
	}

	// Load Blue Laser texture
	if (!gLaserBlueTexture.loadFromFile("LaserBlue.png")) {
		printf("Failed to load Blue Laser texture!\n");
		success = false;
	}

	// Load Ninja Star texture 
	if (!gNinjaStarTexture.loadFromFile("NinjaStar.png")) {
	...
	gLaserFX = Mix_LoadWAV("LaserPlayer.wav");	// Load sound effects
	if (gLaserFX == NULL) {
		printf("Failed to load laser beam sound effect! SDL_mixer Error: %s\n", Mix_GetError());
		success = false;
	}
	gLaserEFX = Mix_LoadWAV("LaserEnemy.wav");	// Load sound effects
	if (gLaserEFX == NULL) {
